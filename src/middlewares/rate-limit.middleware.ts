import { Global, Injectable, NestMiddleware, Scope, HttpException, HttpStatus, Inject } from "@nestjs/common";
...
export class RateLimitMiddleware implements NestMiddleware {
    private readonly rateCounter = new Map<string, { count: number; lastRequest: number; }>();
    ...
    use(req: Request, res: Response, next: NextFunction) {
        ...
        if (!ip) {
            throw new HttpException('Cannot determine IP address', HttpStatus.BAD_REQUEST);
        }
