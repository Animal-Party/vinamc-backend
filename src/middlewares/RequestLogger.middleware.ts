...
import { RequestData, RequestLoggerOptions } from '../types';
...
export class RequestLoggerMiddleware implements NestMiddleware {
    ...
    private logRequest(requestData: RequestData): void {
        ...
        let message = `[${requestId}] ⬅️ ${method} ${originalUrl} - ${ip} - ${userAgent}`;
    ...
    private logResponse(res: Response, requestData: RequestData, hrDuration: [number, number]): void {
        ...
        const logMessage = `[${requestId}] ${logSymbol} ${method} ${originalUrl} ${statusCode} ${contentLength}B - ${durationMs}ms`;
